...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
 	...
 	if(rval==0)	 
	{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			EN25QXX_Write(tempbuf,offx+flashaddr,4096);		//从0开始写入4096个数据  
	  		offx+=bread;	  
...
//			EN25QXX_Read((u8*)buf,((FONTINFOADDR/4096)+i)*4096,4096);//读出整个扇区的内容
//			for(j=0;j<1024;j++)//校验数据
//			{
//				if(buf[j]!=0XFFFFFFFF)break;//需要擦除  	  
//			}
//			if(j!=1024)EN25QXX_Erase_Sector((FONTINFOADDR/4096)+i);	//需要擦除的扇区
//		}
		for(i=0;i<4;i++)	//依次更新UNIGBK,GBK12,GBK16,GBK24,GBK32
		{
